        ...
        wx.createOffscreenCanvas({
          ...
          success: (canvas) => {
            ...
            img.onload = () => {
              ...
              // 绘制水印背景条（半透明黑底）
              const barHeight = Math.max(40, height * 0.06);
              ctx.fillStyle = 'rgba(0, 0, 0, 0.55)';
              ctx.fillRect(0, height - barHeight, width, barHeight);

              // 绘制水印文字
              const fontSize = Math.max(18, Math.floor(barHeight * 0.45));
              ...
              wx.canvasToTempFilePath({
                ...
                quality: 0.88,
                success: (res) => {
                  const mergedPath = res.tempFilePath;

                  // Step 4: wx.uploadFile 上传至后端中转接口
                  const uploadUrl = (app.globalData.baseUrl || '') + '/api/v1/upload/task-photo';
                  wx.uploadFile({
                    url: uploadUrl,
                    filePath: mergedPath,
                    name: 'file',
                ...
                fail: (err) => reject(new Error('Canvas导出失败: ' + JSON.stringify(err))),
              });
            };
            img.onerror = () => reject(new Error('图片加载失败'));
            img.src = tempFilePath;
          },
          fail: (err) => reject(new Error('创建OffscreenCanvas失败: ' + JSON.stringify(err))),
        });
      },
      fail: (err) => reject(new Error('获取图片信息失败: ' + JSON.stringify(err))),
    });
  });
}

module.exports = { composeWatermarkAndUpload };
