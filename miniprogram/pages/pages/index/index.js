...
Page({
  ...
  buildPipelineLines: function () {
    ...
    if (floor === 1) {
      // ═══ F1 封闭循环管线 ═══
      // 机动物流区(底部) → 收脏分拣区(左上)
      lines.push({ id: 'f1_01', x1: '25%', y1: '84%', direction: 'up', label: '装卸' });
      // 收脏分拣区(左上) → 洗涤龙工区(左中)
      lines.push({ id: 'f1_02', x1: '25%', y1: '28%', direction: 'down', label: '' });
      // 收脏分拣区(左上) → 单机洗烘区(左下)
      lines.push({ id: 'f1_03', x1: '15%', y1: '28%', direction: 'down-long', label: '' });
      // 洗涤龙工区(左中) → 展布平烫A(右中)
      lines.push({ id: 'f1_04', x1: '48%', y1: '44%', direction: 'right', label: '' });
      // 单机洗烘区(左下) → 展布平烫B(右下)
      lines.push({ id: 'f1_05', x1: '48%', y1: '70%', direction: 'right', label: '' });
      // 展布平烫A/B → 毛巾折叠区(右上)
      lines.push({ id: 'f1_06', x1: '73%', y1: '32%', direction: 'up', label: '' });
      // 毛巾折叠区(右上) → 机动物流区(底部)
      lines.push({ id: 'f1_07', x1: '73%', y1: '28%', direction: 'down-long', label: '' });
    } else {
      // ═══ F2 精洗区管线 ═══
      // 分拣打标区(左上) → 洗烘区(左中)
      lines.push({ id: 'f2_01', x1: '25%', y1: '32%', direction: 'down', label: '' });
      // 洗烘区(左中) → 手工洗涤区(左下)
      lines.push({ id: 'f2_02', x1: '25%', y1: '62%', direction: 'down', label: '' });
      // 手工洗涤区(左下) → 烘房区(右上) 跨列
      lines.push({ id: 'f2_03', x1: '48%', y1: '79%', direction: 'right-up', label: '' });
      // 烘房区(右上) → 熨烫区(右中)
      lines.push({ id: 'f2_04', x1: '73%', y1: '32%', direction: 'down', label: '' });
      // 熨烫区(右中) → 收发挂衣区(右下)
      lines.push({ id: 'f2_05', x1: '73%', y1: '62%', direction: 'down', label: '' });
    }
  ...
  buildSheetItems: function (zone) {
    var mockItems = {
      ...
      zone_b: [
        { id: 1, name: '水洗单机 #1~#3', sub: '3台运行中', icon: '🔄', iconBg: 'rgba(59,130,246,0.1)', statusKey: 'running', statusLabel: '运行中', value: '3台' },
        { id: 2, name: '水洗单机 #4~#5', sub: '2台待机', icon: '🔄', iconBg: 'rgba(107,114,128,0.1)', statusKey: 'idle', statusLabel: '待机', value: '2台' },
        { id: 3, name: '贯通烘干机 #1', sub: '前进后出 · 运行中', icon: '🔥', iconBg: 'rgba(239,68,68,0.1)', statusKey: 'running', statusLabel: '运行中', value: '82°C' },
        { id: 4, name: '贯通烘干机 #2', sub: '前进后出 · 运行中', icon: '🔥', iconBg: 'rgba(239,68,68,0.1)', statusKey: 'running', statusLabel: '运行中', value: '78°C' },
      ],
      zone_c: [
        { id: 1, name: '8滚烫平机', sub: '温度 185°C（阈值180°C）', icon: '🌡️', iconBg: 'rgba(239,68,68,0.1)', statusKey: 'alert', statusLabel: '告警', value: '185°C' },
        { id: 2, name: '展布机 #1', sub: '正常运行', icon: '📋', iconBg: 'rgba(0,255,136,0.1)', statusKey: 'running', statusLabel: '运行中', value: '正常' },
        { id: 3, name: '展布机 #2', sub: '正常运行', icon: '📋', iconBg: 'rgba(0,255,136,0.1)', statusKey: 'running', statusLabel: '运行中', value: '正常' },
      ],
      zone_d: [
        { id: 1, name: '6滚烫平机', sub: '正常运行', icon: '🌡️', iconBg: 'rgba(0,255,136,0.1)', statusKey: 'running', statusLabel: '运行中', value: '175°C' },
        { id: 2, name: '展布机 #3', sub: '待机中', icon: '📋', iconBg: 'rgba(107,114,128,0.1)', statusKey: 'idle', statusLabel: '待机', value: '' },
      ],
      zone_e: [
      ...
      zone_j: [
        { id: 1, name: '毛巾折叠机 #1', sub: '正常运行', icon: '📦', iconBg: 'rgba(0,255,136,0.1)', statusKey: 'running', statusLabel: '运行中', value: '320/500' },
        { id: 2, name: '毛巾折叠机 #2', sub: '待机中', icon: '📦', iconBg: 'rgba(107,114,128,0.1)', statusKey: 'idle', statusLabel: '待机', value: '' },
      ],
      // ── F2 精洗区 ──
      zone_g: [
        { id: 1, name: '打标机 #1', sub: '运行中', icon: '🏷️', iconBg: 'rgba(0,255,136,0.1)', statusKey: 'running', statusLabel: '运行中', value: '正常' },
        { id: 2, name: '分拣台', sub: '120件已分拣', icon: '🧺', iconBg: 'rgba(236,72,153,0.1)', statusKey: 'running', statusLabel: '使用中', value: '120件' },
      ],
      zone_h: [
        { id: 1, name: '洗烘一体机 #1', sub: '运行中', icon: '🔄', iconBg: 'rgba(0,255,136,0.1)', statusKey: 'running', statusLabel: '运行中', value: '正常' },
        { id: 2, name: '洗烘一体机 #2', sub: '运行中', icon: '🔄', iconBg: 'rgba(0,255,136,0.1)', statusKey: 'running', statusLabel: '运行中', value: '正常' },
        { id: 3, name: '洗烘一体机 #3', sub: '待机中', icon: '🔄', iconBg: 'rgba(107,114,128,0.1)', statusKey: 'idle', statusLabel: '待机', value: '' },
      ],
      zone_i: [
        { id: 1, name: '手工洗涤台 #1', sub: '待机中', icon: '🧤', iconBg: 'rgba(107,114,128,0.1)', statusKey: 'idle', statusLabel: '待机', value: '' },
        { id: 2, name: '手工洗涤台 #2', sub: '待机中', icon: '🧤', iconBg: 'rgba(107,114,128,0.1)', statusKey: 'idle', statusLabel: '待机', value: '' },
      ],
      zone_k: [
        { id: 1, name: '烘房 #1', sub: '温度65°C · 运行中', icon: '🔥', iconBg: 'rgba(245,158,11,0.1)', statusKey: 'running', statusLabel: '运行中', value: '65°C' },
        { id: 2, name: '烘房 #2', sub: '温度62°C · 运行中', icon: '🔥', iconBg: 'rgba(245,158,11,0.1)', statusKey: 'running', statusLabel: '运行中', value: '62°C' },
      ],
      zone_l: [
        { id: 1, name: '熨烫台 #1', sub: '运行中', icon: '♨️', iconBg: 'rgba(0,255,136,0.1)', statusKey: 'running', statusLabel: '运行中', value: '正常' },
        { id: 2, name: '熨烫台 #2', sub: '运行中', icon: '♨️', iconBg: 'rgba(0,255,136,0.1)', statusKey: 'running', statusLabel: '运行中', value: '正常' },
        { id: 3, name: '蒸汽发生器', sub: '待机中', icon: '💨', iconBg: 'rgba(107,114,128,0.1)', statusKey: 'idle', statusLabel: '待机', value: '' },
      ],
      zone_m: [
        { id: 1, name: '挂衣架 A区', sub: '65件已挂', icon: '👔', iconBg: 'rgba(132,204,22,0.1)', statusKey: 'running', statusLabel: '使用中', value: '65/100' },
        { id: 2, name: '收发登记台', sub: '今日收发32单', icon: '📋', iconBg: 'rgba(0,255,136,0.1)', statusKey: 'running', statusLabel: '使用中', value: '32单' },
      ],
    };
